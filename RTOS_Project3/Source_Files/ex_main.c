...
double a = 0, v = 0, p = 0, az = 0, el = 0, roll = 0, vy = 0, vx = 0;
vehicle_info * standardVehicle;

double power = 0;
double gravity = 9.8;
double pi = 3.14;
bool GameOverisTrue = false;
/******************************************************************************
 * @brief Physics - This is the general physics update
 *                  Calculating the (a,v,p,az,el,roll)
 * 					Also the slippage and what not
 * @param timeslice - change in time
 * @return none
 *****************************************************************************/
void Physics (uint16_t timeslice)
{
	power = (double)((double)(SpeedStorage->curSpeed) / 100) *
			(standardVehicle->MaxPower * 1000);
	a = (power / v - gravity);
	v = (timeslice * a);
	p = timeslice * v;
}
...
void LedOutput (void *p_arg)
{
    ...
    while (DEF_ON) {
    	OSFlagPend(&VehicleCtrl,                  /* Wait on this */
					...
					DEF_NULL,
				   &err);                         /* time out */
		OSFlagPost(&GameFlagGroup, (OS_FLAGS)LEDFlag, OS_OPT_POST_FLAG_CLR, &err);
    	if (led1)
    	{
 ...
 * @return none
 *****************************************************************************/
void GameOverTask (void * p_arg)
{
    while (DEF_ON) {
    	if (carShiftx0 < 36 || carShiftx1 > 100) // Radius 6 or Radius 10
    	{
			...
			snprintf(str, MAX_STR_LEN, "Stats:");
			GLIB_drawString(&glibContext,
						  str,
						  strlen(str),
						  CENTER_X - ((GLIB_FONT_WIDTH * strlen(str)) / 4),
...
//    static int16_t curSpeed     = 0;
	...
	while (DEF_ON)
	{
		if(GameOverisTrue == true)
		{
	    	OSFlagPend(&GameFlagGroup,                /* Wait on this */
	    			    GameOverFlag,                 /* Events to check */
			            0,                            /* till all set */
						OS_OPT_PEND_FLAG_SET_ALL,     /* then consume */
						DEF_NULL,
					   &err);                         /* time out */
...
static  void  Ex_MainStartTask (void  *p_arg)
{
    ...
    standardVehicle = (vehicle_info *)malloc(sizeof(vehicle_info));
    standardVehicle->Mass = 1234;
    standardVehicle->MaxPower = 93;
    standardVehicle->TurnRadius = 595;
    standardVehicle->VehicleWidth = 170;
    ...
    OSFlagCreate(&GameFlagGroup, "LED Flag", LEDFlag, &err);
    OSFlagCreate(&GameFlagGroup, "Game Over Flag", GameOverFlag, &err);
